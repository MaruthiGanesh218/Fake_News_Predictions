...
BACKEND_CHUNK_DIR="${BACKEND_DIR}/CHUNK-05"
FRONTEND_CHUNK_DIR="${FRONTEND_DIR}/CHUNK-05"
...
REQUIRED_PATHS=(
  ...
  "scripts/verify-chunk-05.sh"
  "backend/CHUNK-05/README.md"
  "frontend/CHUNK-05/README.md"
)
...
if grep -q "NEWS_PROVIDER" "${PROJECT_ROOT}/backend/.env.example" && \
   grep -q "NEWSAPI_KEY" "${PROJECT_ROOT}/backend/.env.example"; then
  env_placeholders_status="pass"
else
  env_placeholders_status="fail"
  notes+=(".env.example missing NEWS_PROVIDER or NEWSAPI_KEY placeholders.")
fi
...
if [ "${pytest_env_ready:-0}" -eq 1 ]; then
  (cd "${BACKEND_DIR}" && "${backend_python}" -m pytest -q tests/test_news_service.py tests/test_check_news.py) >/tmp/chunk05-pytest.log 2>&1 && pytest_status="pass" || pytest_status="fail"
  if [ "${pytest_status}" != "pass" ]; then
    notes+=("pytest failed; inspect /tmp/chunk05-pytest.log")
  fi
else
  pytest_status="fail"
...
if command -v curl >/dev/null 2>&1 && [ -x "${backend_python}" ]; then
  server_log="$(mktemp 2>/dev/null || echo "${BACKEND_DIR}/uvicorn-chunk05.log")"
  pushd "${BACKEND_DIR}" >/dev/null 2>&1 || true
  "${backend_python}" -m uvicorn app.main:app --port 8010 --log-level warning >"${server_log}" 2>&1 &
  server_pid=$!
  popd >/dev/null 2>&1 || true

  ready=0
  for _ in {1..12}; do
    if curl --fail --silent --max-time 3 http://127.0.0.1:8010/health | grep -q '"status"'; then
      ready=1
      break
    fi
    sleep 0.5
...
{"text":"Sample headline about renewable energy"}
EOF
    response=$(curl --silent --show-error --fail --max-time 5 -H "Content-Type: application/json" -d "${CURL_BODY}" http://127.0.0.1:8010/check-news 2>/tmp/chunk05-post.log || true)
    if [ -n "${response}" ] && echo "${response}" | grep -q '"sources"'; then
      post_check_status="pass"
    else
      post_check_status="fail"
      notes+=("POST /check-news did not return sources; see ${server_log} and /tmp/chunk05-post.log")
    fi
  else
    post_check_status="fail"
...
for path in "${created_files[@]}"; do
  escaped_path=${path//\"/\\\"}
  created_json+="\"${escaped_path}\"," # escape quotes
done
...
cat >"${RESULT_FILE_BACKEND}" <<EOF
{
  ...
  "verification": {
    ...
    "post_check_news_sources": "${post_check_status}",
    "notes": "${escaped_notes}"
  },
  "status": "${status}"
}
